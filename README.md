...
Classic encoding (`-x3dv`). XML X3D is generated by converting the native
Classic X3D scene through `view3dscene`. The `-webgl` mode writes an HTML
viewer that embeds the converted XML X3D scene and displays it in the browser
...
`-x3dv` is written natively by MolScriptSVG. `-x3d` and `-webgl` require
`view3dscene` to be installed locally, because the program uses it to convert
the generated Classic X3D scene into XML X3D. The `-webgl` output is a
self-contained HTML wrapper around that converted XML scene, but it still loads
the X_ITE JavaScript runtime from a CDN at view time.
...
* XML X3D and WebGL now use `view3dscene` for reliable Classic-to-XML
  conversion.
