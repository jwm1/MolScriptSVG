...
char *requested_output_filename = NULL;
...
process_arguments (int *argcp, char *argv[])
{
  ...
  if (slot) {
    ...
    fprintf (stderr, "-x3dv                X3D Classic file output\n");
    fprintf (stderr, "-webgl               HTML WebGL viewer output\n");
#ifdef OPENGL_SUPPORT
  ...
  slot = args_exists ("-x3d");
  slot2 = args_exists ("-x3dv");
  if (slot || slot2) {
    if (output_mode != UNDEFINED_MODE) goto format_error;
    if (slot) args_flag (slot);
    if (slot2) args_flag (slot2);
    x3d_set_webgl_output (FALSE);
    x3d_set_xml_encoding (slot != 0);
    x3d_set();
  }

  slot = args_exists ("-webgl");
  if (slot) {
    if (output_mode != UNDEFINED_MODE) goto format_error;
    args_flag (slot);
    x3d_set_webgl_output (TRUE);
    x3d_set();
  }
