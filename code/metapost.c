...
static FILE *mp_body = NULL;
static int mp_plot_count = 0;
static int mp_axes_enabled = FALSE;
static double mp_tick_spacing = 5.0;

#define MP_SEAM_STROKE_WIDTH 0.35
...
mp_ensure_body (void)
{
  if (mp_body == NULL) {
    mp_body = tmpfile();
    if (mp_body == NULL) yyerror ("could not create a temporary MetaPost file");
  }
}
...
/*------------------------------------------------------------*/
static void
mp_write_colour (FILE *file, colour *c)
{
  colour rgb;
  ...
  rgb = *c;
  colour_to_rgb (&rgb);
  fprintf (file, "(%.4f,%.4f,%.4f)", rgb.x, rgb.y, rgb.z);
}


/*------------------------------------------------------------*/
static void
mp_write_pen (FILE *file, colour *stroke, double linewidth, double dash)
{
  assert (file);
  assert (stroke);
  ...
  if (dash >= 0.01) fprintf (file, " dashed evenly scaled %.4f", dash);
  fprintf (file, " withpen pencircle scaled %.4f withcolor ", linewidth);
  mp_write_colour (file, stroke);
}


/*------------------------------------------------------------*/
static void
mp_write_escaped (FILE *file, char *str, int len, int strip_percent)
{
  int slot;
  char ch;
...
/*------------------------------------------------------------*/
static void
mp_write_line (FILE *file, vector3 *v1, vector3 *v2, colour *stroke,
                double linewidth, double dash)
{
  ...
  fprintf (file, "draw (%.4f,%.4f)--(%.4f,%.4f)",
           v1->x, v1->y, v2->x, v2->y);
  mp_write_pen (file, stroke, linewidth, dash);
  fputs (";\n", file);
}
...
/*------------------------------------------------------------*/
static void
mp_write_path_points (FILE *file, vector3 *points, int count)
{
  int slot;
...
/*------------------------------------------------------------*/
static void
mp_write_polygon (FILE *file, vector3 *points, int count,
                   ...
                   double linewidth, double dash)
{
  ...
  if (fill) {
    fputs ("fill ", file);
    mp_write_path_points (file, points, count);
    fputs ("--cycle withcolor ", file);
    mp_write_colour (file, fill);
    fputs (";\n", file);
  }
...
/*------------------------------------------------------------*/
static void
mp_write_filled_polygon (FILE *file, vector3 *points, int count, colour *fill)
{
  assert (file);
  assert (points);
  assert (count >= 3);
  ...
  mp_write_polygon (file, points, count, fill, fill, MP_SEAM_STROKE_WIDTH, 0.0);
}


/*------------------------------------------------------------*/
static void
mp_write_circle (FILE *file, vector3 *center, double radius,
                  colour *stroke, colour *fill,
                  double linewidth, double dash)
{
  assert (file);
  ...
  if (fill) {
    fprintf (file, "fill fullcircle scaled %.4f shifted (%.4f,%.4f) withcolor ",
             2.0 * radius, center->x, center->y);
    mp_write_colour (file, fill);
    fputs (";\n", file);
  }
  if (stroke && linewidth > 0.0) {
    fprintf (file, "draw fullcircle scaled %.4f shifted (%.4f,%.4f)",
             2.0 * radius, center->x, center->y);
    mp_write_pen (file, stroke, linewidth, dash);
    fputs (";\n", file);
  }
...
/*------------------------------------------------------------*/
static void
mp_point_on_circle (vector3 *point, vector3 *center, double radius,
                     double degrees)
{
  double radians;
...
/*------------------------------------------------------------*/
static void
mp_write_arc (FILE *file, vector3 *center, double radius,
               ...
               int clockwise, colour *stroke, double linewidth)
{
  ...
  assert (stroke);
  fprintf (file,
           "%% arc omitted in MetaPost backend: center=(%.4f,%.4f) r=%.4f start=%.2f end=%.2f cw=%d\n",
           center->x, center->y, radius, start_degrees, end_degrees, clockwise);
}


/*------------------------------------------------------------*/
static void
mp_write_donald_duck (FILE *file, vector3 *center, double radius)
{
  assert (file);
  assert (center);
  assert (radius >= 0.0);
  fprintf (file,
           "%% Donald Duck sphere highlights omitted in MetaPost backend at (%.4f,%.4f) r=%.4f\n",
...
/*------------------------------------------------------------*/
static void
mp_write_label_text (FILE *file, char *str, int len, state *st)
{
  assert (file);
  assert (str);
  assert (len > 0);
  assert (st);
  mp_write_escaped (file, str, len, TRUE);
}


/*------------------------------------------------------------*/
static void
mp_draw_axes (FILE *file, double matrix[4][4], double scale)
{
  ...
  int tick;
  int tick_count;
  colour axis_colour = black_colour;

  if (! mp_axes_enabled) return;

  axis_limit = window;
  if (slab > axis_limit) axis_limit = slab;
  if (axis_limit < 5.0) axis_limit = 5.0;

  spacing = mp_tick_spacing;
  if (spacing <= 0.0) spacing = 5.0;
  tick_count = (int) floor (axis_limit / spacing);
  tick_length = 4.0;

  matrix3_transform (&origin, matrix);
  mp_comment ("axes with distance ticks (Angstrom)");

  for (axis = 0; axis < 3; axis++) {
    v3_initialize (&axis_end, 0.0, 0.0, 0.0);
    if (axis == 0) axis_end.x = axis_limit;
    if (axis == 1) axis_end.y = axis_limit;
    if (axis == 2) axis_end.z = axis_limit;
    matrix3_transform (&axis_end, matrix);

    mp_write_line (file, &origin, &axis_end, &axis_colour, 0.8, 0.0);
    fprintf (file,
             "label(\"%s\", (%.4f,%.4f));\n",
             axis_names[axis], axis_end.x + 6.0, axis_end.y + 6.0);

    dir2d.x = axis_end.x - origin.x;
    dir2d.y = axis_end.y - origin.y;
    dir2d.z = 0.0;
    if (v3_length (&dir2d) < 1.0e-6) continue;
    v3_normalize (&dir2d);
    v3_initialize (&perp2d, -dir2d.y, dir2d.x, 0.0);

    for (tick = 1; tick <= tick_count; tick++) {
      v3_initialize (&tick_point, 0.0, 0.0, 0.0);
      if (axis == 0) tick_point.x = tick * spacing;
      if (axis == 1) tick_point.y = tick * spacing;
      if (axis == 2) tick_point.z = tick * spacing;
      matrix3_transform (&tick_point, matrix);

      tick_left = tick_point;
      tick_right = tick_point;
      tick_left.x -= 0.5 * tick_length * perp2d.x;
      tick_left.y -= 0.5 * tick_length * perp2d.y;
      tick_right.x += 0.5 * tick_length * perp2d.x;
      tick_right.y += 0.5 * tick_length * perp2d.y;
      mp_write_line (file, &tick_left, &tick_right, &axis_colour, 0.6, 0.0);
      fprintf (file,
               "label(\"%d\", (%.4f,%.4f));\n",
               (int) (tick * spacing),
               tick_point.x + 4.0 * perp2d.x + 3.0,
               tick_point.y + 4.0 * perp2d.y + 3.0);
    }
  }
}
...
db_plane_output (depth_db_entry *de, char *plane_type)
{
  ...
  if (str_eq ("P", plane_type)) {
    mp_write_filled_polygon (file, points, 4, &(pe->col));
  } else {
...
db_tri_output (depth_db_entry *de, char *tri_type)
{
  ...
  points[1] = pe->p2;
  points[2] = pe->p3;
  mp_write_filled_polygon (file, points, 3, &(pe->col));
  if (! str_eq ("T", tri_type)) {
    mp_write_line (file, &(pe->p1), &(pe->p2), &(de->st->linecolour),
...
db_label_output (depth_db_entry *de)
{
  ...
  fprintf (file, "label(\"");
  mp_write_label_text (file, lae->str, len, de->st);
  fprintf (file, "\", (%.2f,%.2f));\n", lae->pos.x, lae->pos.y);
...
db_output (void)
{
  ...
  for (slot = 0; slot < depth_count; slot++) {
    ...
    case LINE_CODE:
      ...
      mp_write_line (file, &(le->v1), &(le->v2),
    ...
    case SPHERE_CODE:
      ...
      mp_write_circle (file, &(se->v), se->rad,
                        &(de->st->linecolour), &(se->col),
                        depthcue (de->depth, de->st) * de->st->linewidth,
                        de->st->linedash);
      if (se->donald_duck) mp_write_donald_duck (file, &(se->v), se->rad);
      sphere_output_count++;
      break;
    ...
    case STICK_CODE:
      ...
      mp_write_polygon (file, points, 4, &(de->st->linecolour), &(ste->col),
                         depthcue (de->depth, de->st) * de->st->linewidth,
                         de->st->linedash);
      stick_output_count++;
      break;
...
mp_set (void)
{
  output_first_plot = mp_first_plot;
  output_finish_output = mp_finish_output;
  output_start_plot = mp_start_plot;
  output_finish_plot = mp_finish_plot;

  set_area = mp_set_area;
  set_background = mp_set_background;
  ...
  output_comment = mp_comment;

  output_coil = mp_coil;
  output_cylinder = mp_cylinder;
  output_helix = mp_helix;
  output_label = mp_label;
  output_line = mp_line;
  output_sphere = mp_sphere;
  output_stick = mp_stick;
  output_strand = mp_strand;

  output_start_object = do_nothing;
  output_object = mp_object;
  output_finish_object = do_nothing;

  output_pickable = NULL;

  output_mode = METAPOST_MODE;
}


/*------------------------------------------------------------*/
void
mp_set_axes (int enabled)
{
  mp_axes_enabled = enabled;
}
...
/*------------------------------------------------------------*/
void
mp_finish_output (void)
{
  int ch;

  set_outfile ("w");
  fprintf (outfile, "%% MetaPost generated by %s\n", program_str);
  fprintf (outfile, "%% %s\n", copyright_str);
  if (title) {
    fprintf (outfile, "%% title: ");
    mp_write_escaped (outfile, title, strlen (title), FALSE);
    fputc ('\n', outfile);
  }
  fputs ("prologues := 3;\n", outfile);
  fputs ("outputtemplate := \"%j-%c.mps\";\n\n", outfile);

  rewind (mp_body);
  while ((ch = fgetc (mp_body)) != EOF) fputc (ch, outfile);
...
mp_finish_plot (void)
{
  ...
  mp_write_colour (file, &background_colour);
  fputs (";\n", file);

  mp_draw_axes (file, matrix, scale);
  db_output();
  fputs ("endfig;\n\n", file);
...
/*------------------------------------------------------------*/
void
mp_cylinder (vector3 *v1, vector3 *v2)
{
  int slot, slices;
...
/*------------------------------------------------------------*/
void
mp_label (vector3 *p, char *label, colour *c)
{
  label_db_entry *lae;
...
/*------------------------------------------------------------*/
void
mp_sphere (at3d *at, double radius)
{
  assert (at);
  assert (radius > 0.0);
...
/*------------------------------------------------------------*/
void
mp_stick (vector3 *v1, vector3 *v2, double r1, double r2, colour *c)
{
...
/*------------------------------------------------------------*/
void
mp_object (int code, vector3 *triplets, int count)
{
  int slot;
  vector3 *v;
  vector3 normal;
